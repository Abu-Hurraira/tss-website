...
export function Navbar({ className }: { className?: string }) {
  return (
    <nav className={cn('hidden items-center gap-1 lg:flex', className)} aria-label="Primary">
      <NavLink
        ...
        className={({ isActive }) =>
          cn(
            ...
            isActive ? 'bg-brand-muted text-brand' : 'text-ink-secondary hover:bg-brand-soft hover:text-navy',
          )
        }
      >
        Home
      ...
      <Link
        to="/site/apply"
        className="ml-2 rounded-full bg-brand px-5 py-2.5 text-sm font-bold text-white shadow-[0_8px_20px_rgb(12_85_143/0.25)] transition hover:bg-brand-hover"
      >
        Apply Now
      </Link>
    </nav>
